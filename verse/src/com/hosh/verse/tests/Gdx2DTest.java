...
public class Gdx2DTest extends GdxTest {
	...
	public void create() {
		...
		final Gdx2DPixmap[] pixmaps = testPixmaps();

		final Gdx2DPixmap composite = new Gdx2DPixmap(512, 256, Gdx2DPixmap.GDX2D_FORMAT_RGBA8888);
		composite.clear(0);
		...
		for (int i = 0; i < pixmaps.length; i++) {
			...
			composite.drawPixmap(pixmaps[i], 0, 0, 32, 32, i * 64, 100, 64, 64);
			composite.drawPixmap(pixmaps[i], 0, 0, 32, 32, i * 64, 164, 16, 16);
			final Sprite sprite = new Sprite(textureFromPixmap(pixmaps[i]));
			sprite.setPosition(10 + i * 32, 10);
			sprites.add(sprite);
		}

		final Sprite sprite = new Sprite(textureFromPixmap(composite));
