...
public class DatabaseAccessor {
	...
	public static VerseActor loadBaseType(final Connection dbConnection, final String id) {
		PreparedStatement stmt;
		try {
			stmt = dbConnection.prepareStatement("SELECT * FROM base_type WHERE id=?");
			stmt.setString(1, id);
			...
			final int npcId = res.getInt(VerseActor.DBID_BASE_TYPE_ID);
			final String name = res.getString(VerseActor.DBID_BASE_TYPE_NAME);
			final int typeId = res.getInt(VerseActor.DBID_BASE_TYPE_TPYE_ID);
			final int modelId = res.getInt(VerseActor.DBID_BASE_TYPE_MODEL_ID);
			final int socialRange = res.getInt(VerseActor.DBID_BASE_TYPE_SOCIAL);
			final int aggroRange = res.getInt(VerseActor.DBID_BASE_TYPE_AGGRO);
			final int colorR = res.getInt(VerseActor.DBID_BASE_TYPE_COLOR_R);
			final int colorG = res.getInt(VerseActor.DBID_BASE_TYPE_COLOR_G);
			final int colorB = res.getInt(VerseActor.DBID_BASE_TYPE_COLOR_B);
			final int colorA = res.getInt(VerseActor.DBID_BASE_TYPE_COLOR_A);
			final int collisionRadius = res.getInt(VerseActor.DBID_BASE_TYPE_COLLISION_RADIUS);
			final int attackRange = res.getInt(VerseActor.DBID_BASE_TYPE_ATTACK_RANGE);
			final int baseHp = res.getInt(VerseActor.DBID_BASE_TYPE_BASE_HP);
			final int baseShield = res.getInt(VerseActor.DBID_BASE_TYPE_BASE_SHIELD);
			final int baseSpeed = res.getInt(VerseActor.DBID_BASE_TYPE_BASE_SPEED);
			final int baseAttack = res.getInt(VerseActor.DBID_BASE_TYPE_BASE_ATTACK);
			final int baseDefense = res.getInt(VerseActor.DBID_BASE_TYPE_BASE_DEFENSE);
			final int extensionSlots = res.getInt(VerseActor.DBID_BASE_TYPE_EXTENSION_SLOTS);
